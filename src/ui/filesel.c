...
static char **
ui_mksnames (int nnames, char **names, int width)
{
    char **snames = NULL;
    int i;
    if (nnames) {
...
static void
drawfile (uih_context * c, void *data)
{
    int i;
    int ypos;
    ...
    ui_drawbutton (gettext ("Cancel"), (pressedbutton == 1), active == AOK && activebutton == 1, filex + filewidth / 2 + BORDERWIDTH, filex + filewidth - BORDERWIDTH, OKSTART);
    ...
    for (i = 0; ypos + h < LISTEND && i + namestart < nnames; i++) {
        ...
        if (uih->palette->type & BITMAPS)
            xprint (uih->image, uih->font, filex + 2 * BORDERWIDTH, ypos, snames[i + namestart], i + namestart == selectedname ? FGCOLOR (uih) : BGCOLOR (uih), BGCOLOR (uih), TEXT_PRESSED);
        else
    ...
    for (i = 0; ypos + h < LISTEND && i + dirstart < ndirs; i++) {
        ...
        if (uih->palette->type & BITMAPS)
            xprint (uih->image, uih->font, filex + filewidth / 2 + 2 * BORDERWIDTH, ypos, sdirs[i + dirstart], i + dirstart == selecteddir ? FGCOLOR (uih) : BGCOLOR (uih), BGCOLOR (uih), TEXT_PRESSED);
        else
...
setdir (int name)
{
    ...
    free (currdir);
#ifdef DRIVES
    if (strlen (s) == 2 && ((s[0] >= 'a' && s[0] <= 'z') || (s[0] >= 'A' && s[0] <= 'Z')) && s[1] == ':')
...
static void
setexactdir (const char *dirstring)
{
    free (currdir);
...
ui_mousefilesel (int x, int y, int buttons, int flags)
{
    ...
    if (y < LISTSTART) {
        ...
        if (x > LISTWIDTH && (flags & MOUSE_PRESS))
            ...
            if (flags & MOUSE_PRESS) {
                int atitem = (y - LISTSTART - BORDERHEIGHT) / xtextheight (uih->image,
                                                                           uih->font);
                if (atitem < 0)
                    atitem = 0;
                if (!mouseat) {
...
void
ui_buildfilesel (const char *f, const char *m, void (*c) (const char *, int))
{
    if (filevisible) {
