...
static int
getwidth (void *data, int flags, const char *text)
{
...
static void
helpsize (struct uih_context *c, int *x, int *y, int *width, int *height, void *data)
{
    *x = helpx;
    *y = helpy;
    *width = helpwidth;
...
static void
drawhelp (struct uih_context *c, void *data)
{
    int i = 0;
    int y;
    ...
    while ((y = YPOS (lines[i].y)) < helpheight - HBORDERS - xtextheight (uih->image, uih->font) && lines[i].y >= 0) {
            ...
            if (curritem->c.linktext != NULL) {
                if (uih->palette->type & BITMAPS) {
                    if (curritem == presseditem || curritem == selecteditem)
                        fgcolor = FGCOLOR (uih), xrectangle (uih->image, x + helpx, y + helpy, curritem->width, xtextheight (uih->image, uih->font), BGCOLOR (uih));
                    else
                        xhline (uih->image, x + helpx, y + helpy + xtextheight (uih->image, uih->font) - 1, curritem->width, BGCOLOR (uih));
                } else {
                    if (uih->image->flags & AAIMAGE)
                        ...
                        if (i > 3 && !strcmp (".xaf", curritem->c.linktext + i - 4))
                            xhline (uih->image, x + helpx, y + helpy + xtextheight (uih->image, uih->font) - 1, curritem->width, curritem == presseditem || curritem == selecteditem ? SELCOLOR (uih) : SELCOLOR (uih));
                        else
                            xhline (uih->image, x + helpx, y + helpy + xtextheight (uih->image, uih->font) - 1, curritem->width, curritem == presseditem || curritem == selecteditem ? SELCOLOR (uih) : LIGHTGRAYCOLOR2 (uih));
                        xhline (uih->image, x + helpx + 1, y + helpy + xtextheight (uih->image, uih->font) - 0, curritem->width, BGCOLOR (uih));
                    }
                    flags = 0;
                    bgcolor = BGCOLOR (uih);
...
ui_helpkeys (int key)
{
    ...
    switch (key) {
        ...
        case 'f':
        case ' ':
            ui_helppos[historypos] += helpheight - HBORDERS - 32, uih->display = 1;
            presseditem = NULL;
            selecteditem = NULL;
            break;
        case UIKEY_PGUP:
...
ui_helpmouse (int x, int y, int buttons, int flags)
{
    ...
    if (x < helpx || y < helpy || x > helpx + helpwidth || y > helpy + helpheight) {
        ...
        if (y > helpy + helpheight - BUTTONHEIGHT - 2 * BORDERHEIGHT) {
            ...
            button = i - 1;
            if (flags & MOUSE_DRAG) {
                if (pressedbutton != selectedbutton || pressedbutton != button)
                    pressedbutton = selectedbutton = button, uih->display = 1;
            } else {
                if (pressedbutton != -1)
                    pressedbutton = -1, uih->display = 1;
        ...
        pressedbutton = -1;
        if (presseditem != NULL) {
            if (YPOS (lines[pressedline].y) + 1 >= y && YPOS (lines[pressedline].y) <= y + xtextheight (uih->image, uih->font) && presseditem->x <= x && presseditem->x + presseditem->width >= x)
                atpressed = 1;
        }
        if (flags & MOUSE_PRESS || ((flags & MOUSE_DRAG) && !atpressed)) {
            int i = 0;
            for (i = 0; lines[i].y >= 0 && (YPOS (lines[i].y) + 1 <= y || YPOS (lines[i].y) >= y + xtextheight (uih->image, uih->font)); i++);
            if (lines[i].y >= 0) {
...
ui_build_help (char *name)
{
    ...
    if (width > WIDTH)
        width = WIDTH;
    lines = help_make (name ? name : "main", getwidth, width - 2, xtextheight (uih->image, uih->font), xtextheight (uih->image, uih->font));
    if (lines == NULL) {
        lines = help_make ("main", getwidth, width - 2, xtextheight (uih->image, uih->font), xtextheight (uih->image, uih->font));
        if (lines == NULL) {
            helpvisible = 0;
