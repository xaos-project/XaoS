...
{
    dialog.items[dialog.current].type->unselect (dialog.items + dialog.current);
    dialog.current = dialog.current ? (dialog.current - 1) : dialog.nitems - 1;
    uih->display = 1;
}
...
ui_drawbutton (const char *text, int pressed, int selected, int x1, int x2, int y)
{
    ...
    if (uih->palette->type & BITMAPS) {
        ...
        xprint (uih->image, uih->font, (x1 + x2 - width) / 2 + pressed, y + BORDERHEIGHT + pressed, text, selected ? SELCOLOR (uih) : FGCOLOR (uih), BGCOLOR (uih),
                /*TEXT_PRESSED */ 0);
    }
}
...
ui_drawyesno (struct uih_context *c, void *data)
{
    ...
    ui_drawbutton (yestext, yesnodialog.pressed == 0, yesnodialog.selected == 0, YESNOX + BORDERWIDTH + 1, YESNOX + (yesnodialog.width) / 2 - 1, YESNOY + BUTTONHEIGHT + BORDERHEIGHT);
    ui_drawbutton (notext, yesnodialog.pressed == 1, yesnodialog.selected == 1, YESNOX + (yesnodialog.width) / 2 + 1, YESNOX + yesnodialog.width - BORDERWIDTH - 1, YESNOY + BUTTONHEIGHT + BORDERHEIGHT);
}
...
ui_buildyesno (const char *question, void (*handler) (int yes))
{
    ...
    yesnodialog.width = xtextwidth (uih->image, uih->font, yestext) + xtextwidth (uih->image, uih->font, notext) + 8 * BORDERWIDTH + 2;
...
static int
ui_mouseyesno (int x, int y, int buttons, int flags)
{
    int mouseat = 0;
    if (!yesnodialogvisible)
        return 0;
...
static void
ui_buildok (struct dialogitem *item, const menudialog * entry)
{
    struct okdata *ok;
...
static void
ui_destroyok (struct dialogitem *item, dialogparam * param)
{
    free (item->data);
}
...
{
    struct okdata *ok = (struct okdata *) item->data;
    ui_drawbutton (gettext ("OK"), ok->pressed == 0, SELECTED (item) && ok->selected == 0, dialog.x + BORDERWIDTH + 1, dialog.x + (dialog.width) / 3 - 1, item->y);
    ui_drawbutton (gettext ("Cancel"), ok->pressed == 1, SELECTED (item) && ok->selected == 1, dialog.x + (dialog.width) / 3 + 1, dialog.x + 2 * dialog.width / 3 - BORDERWIDTH, item->y);
    ui_drawbutton (gettext ("Help"), ok->pressed == 2, SELECTED (item) && ok->selected == 2, dialog.x + 2 * (dialog.width) / 3, dialog.x + dialog.width - BORDERWIDTH - 1, item->y);
}
...
void
ui_updatetext (struct ui_textdata *d)
{
    int again = 1;
    int i;
    int wi;
...
ui_drawtext (struct ui_textdata *d, int active)
{
    ...
    c[d->ndisplayed] = 0;
    xprint (uih->image, uih->font, d->x, d->y, c, (uih->palette->type & BITMAPS) ? BGCOLOR (uih) : ((active && d->clear) ? SELCOLOR (uih) : FGCOLOR (uih)), BGCOLOR (uih), (uih->palette->type & BITMAPS) ? TEXT_PRESSED : 0);
    if (active) {
        xdrawcursor (uih->image, d->x + d->cursorpos, d->y, (uih->palette->type & BITMAPS) ? BGCOLOR (uih) : SELCOLOR (uih), xtextheight (uih->image, uih->font));
    }
    free (c);
}
...
void
ui_closetext (struct ui_textdata *d)
{
    free (d->text);
    free (d);
}
...
static void
ui_buildint (struct dialogitem *item, const menudialog * entry)
{
    char s[50];
    item->height = BUTTONHEIGHT;
    item->width = xtextwidth (uih->image, uih->font, item->dialog->question);
...
ui_drawfile (struct dialogitem *item)
{
    ...
    if (data->text->width == 2043) {
        data->text->x = dialog.half + BORDERWIDTH, data->text->width = wholesize - BROWSEWIDTH - 2 * BORDERWIDTH, data->text->y = item->y + BORDERHEIGHT;
    ...
    xprint (uih->image, uih->font, dialog.half - item->width, item->y + BORDERHEIGHT, item->dialog->question, SELECTED (item) ? SELCOLOR (uih) : FGCOLOR (uih), BGCOLOR (uih), 0);
    ui_drawquestion (item);
    ui_drawbutton ("B", data->pressed && SELECTED (item), SELECTED (item) && data->active, dialog.x + dialog.width - BROWSEWIDTH - BORDERWIDTH, dialog.x + dialog.width - BORDERWIDTH, item->y);
}
...
ui_drawcoord (struct dialogitem *item)
{
    ...
    if (data->text[0]->width == 2043) {
        data->text[0]->x = dialog.half + BORDERWIDTH, data->text[0]->width = half - BORDERWIDTH, data->text[0]->y = item->y + BORDERHEIGHT;
        data->text[1]->x = dialog.half + half + SPACESIZE + BORDERWIDTH, data->text[1]->width = half - BORDERWIDTH, data->text[1]->y = item->y + BORDERHEIGHT;
...
ui_drawchoicemenu (uih_context * uih, void *data)
{
    ...
    for (i = 0; i < choice->n; i++) {
        xprint (uih->image, uih->font, choice->x + BORDERWIDTH, choice->y + BORDERHEIGHT + i * xtextheight (uih->image, uih->font), choice->texts[i], i == choice->active ? SELCOLOR (uih) : FGCOLOR (uih), BGCOLOR (uih), 0);
    }
}
...
ui_builddialog (const menuitem * item)
{
    ...
    n = xtextwidth (uih->image, uih->font, gettext ("OK")) + xtextwidth (uih->image, uih->font, gettext ("Cancel")) + xtextwidth (uih->image, uih->font, gettext ("Help")) + 10;
    if (dialog.width < n)
        dialog.width = n;
...
static void
ui_dialogquestion (int succesfull)
{
    if (succesfull)
...
int
ui_dialogmouse (int x, int y, int mousebuttons, int flags)
{
    int i;
