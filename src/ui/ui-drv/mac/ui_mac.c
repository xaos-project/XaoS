...
void
SetWindPaletteFromClut (WindowPtr theWindow, CTabHandle theClut)
{
...
display_refresh ()
{
  ...
  if (mac_show_grow_icon)
    {
      ...
      CopyBits (&((GrafPtr) mac_offscreen[mac_currentbuff])->portBits,
		&((GrafPtr) mac_grow_icon_gworld)->portBits, &r,
		&mac_grow_icon_gworld->portRect, srcCopy, nil);
  ...
  CopyBits (&((GrafPtr) mac_offscreen[mac_currentbuff])->portBits,
	    &((GrafPtr) mac_screen)->portBits,
	    &mac_offscreen[mac_currentbuff]->portRect, &mac_screen->portRect,
	    srcCopy, nil);
  ...
  if (mac_show_grow_icon)
    {
      ...
      CopyBits (&((GrafPtr) mac_grow_icon_gworld)->portBits,
		&((GrafPtr) mac_offscreen[mac_currentbuff])->portBits,
		&mac_grow_icon_gworld->portRect, &r, srcCopy, nil);
...
static void
mac_print (int x, int y, CONST char *text)
{
...
static int
mac_set_color (int r, int g, int b, int init)
{
...
mac_init (int fullScreen)
{
  ...
  if (full_screen_mode)
	...
	r.top += GetMBarHeight ();
      ...
      mac_screen =
	NewCWindow (nil, &r, (StringPtr) winTitle, true, documentProc,
		    (WindowPtr) - 1, true, nil);
...
static void
mac_getmouse (int *x, int *y, int *buttons)
{
...
static void
mac_processevents (int wait, int *x, int *y, int *buttons, int *k)
{
  EventRecord event;
  char key;
  char vkey;
	...
	case mouseDown:
	  ...
	  if (hitWindow != mac_screen)
		...
		case inGrow:
		  ...
		  newWinSize =
		    GrowWindow (mac_screen, event.where, &rSizeRect);
		  if (newWinSize != 0)
		    {
		      mac_window_width = LoWord (newWinSize);
		      mac_window_height = HiWord (newWinSize);
		      SizeWindow (mac_screen, mac_window_width,
				  mac_window_height, false);
		      // Switch to black help text since resized window wipes itself white.
		      mac_text_color.red = mac_text_color.green =
			mac_text_color.blue = 0x0000;
		      ui_call_resize ();
...
static int
mac_alloc_buffers (char **buffer1, char **buffer2)
{
  int rowBytes = 0;
  PixMapHandle pmap;
  OSErr err1, err2;
  ...
  if (err1 != noErr || err2 != noErr)
    {
      if (mac_offscreen[0] != nil)
	{
	  DisposeGWorld (mac_offscreen[0]);
	  mac_offscreen[0] = nil;
	}
      if (mac_offscreen[1] != nil)
	{
	  DisposeGWorld (mac_offscreen[1]);
	  mac_offscreen[1] = nil;
	}
      return 0;
    }
...
static void
mac_free_buffers (char *buffer1, char *buffer2)
{
