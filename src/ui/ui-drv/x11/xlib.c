...
#ifdef MITSHM
    if (d->SharedMemFlag) {
        XShmPutImage (d->display, d->window, d->gc, d->image[d->current], 0, 0, 0, 0, d->bwidth, d->bheight, True);
    } else
#endif
...
alloc_shm_image (xdisplay * new)
{
    ...
    if (!ptr || (*ptr == ':') || !strncmp (ptr, "localhost:", 10) || !strncmp (ptr, "unix:", 5) || !strncmp (ptr, "local:", 6)) {
    ...
    for (i = 0; i < 2; i++) {
        if (new->SharedMemOption) {
            new->SharedMemFlag = False;
            new->image[i] = XShmCreateImage (new->display, new->visual, new->depth, new->depth == 1 ? XYBitmap : ZPixmap, NULL, &new->xshminfo[i], new->width, new->height);
            if (new->image[i]) {
...
alloc_image (xdisplay * d)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        d->image[i] = XCreateImage (d->display, d->visual, d->depth, d->depth == 1 ? XYBitmap : ZPixmap, 0, NULL, d->width, d->height, 32, 0);
        if (d->image[i] == NULL) {
        ...
        }
        {
            ...
            *(unsigned short *) c = 0xff;
            if ((!(d->image[i]->red_mask & ~0xffU) || !(d->image[i]->red_mask & ~0xff00U) || !(d->image[i]->red_mask & ~0xff0000U) || !(d->image[i]->red_mask & ~0xff000000U)) && (!(d->image[i]->green_mask & ~0xffU) || !(d->image[i]->green_mask & ~0xff00U) || !(d->image[i]->green_mask & ~0xff0000U) || !(d->image[i]->green_mask & ~0xff000000U)) && (!(d->image[i]->blue_mask & ~0xffU) || !(d->image[i]->blue_mask & ~0xff00U) || !(d->image[i]->blue_mask & ~0xff0000U) || !(d->image[i]->blue_mask & ~0xff000000U)))
                byteexact = 1;
            if (!byteexact) {
...
xdisplay *
xalloc_display (CONST char *s, int x, int y, xlibparam * params)
{
    xdisplay *new;
    ...
    new->attributes->event_mask = ButtonPressMask | StructureNotifyMask | ButtonReleaseMask | PointerMotionMask | KeyPressMask | ExposureMask | KeyReleaseMask;
    ...
    for (i = 8; i && !found; i--)
        if (XMatchVisualInfo (new->display, new->screen, i, GrayScale, &vis)) {
            found = 1;
        }
    ...
    if (params->rootwindow)
        ...
        new->window = XCreateWindow (new->display, new->parent_window, 0, 0, new->width, new->height, new->border_width, new->depth, new->class, new->visual, new->attr_mask, new->attributes);
...

    {
        ...
        if (fake == 0)
            if (strcmp (prog_argv[prog_argc - 2], "-driver") && strcmp (prog_argv[prog_argc - 1], "x11"))
                fake = 1;
        ...
        if (fake) {
            ...
            XSetWMProperties (new->display, new->window, NULL, NULL, faked_argv, prog_argc + 2, NULL, hints, &classHint);

            free (faked_argv);
        } else
            XSetWMProperties (new->display, new->window, NULL, NULL, prog_argv, prog_argc, NULL, hints, &classHint);
    }
    ...
    XSetBackground (new->display, new->gc, BlackPixel (new->display, new->screen));
    XSetForeground (new->display, new->gc, WhitePixel (new->display, new->screen));
...
void
xouttext (xdisplay * d, CONST char *string)
{
    int sz;

    sz = (int) strlen (string);
    XDrawImageString (d->display, d->window, d->gc, d->lastx, d->lasty, string, sz);
}
