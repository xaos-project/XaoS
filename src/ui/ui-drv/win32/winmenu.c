...
win32_createmenu (struct uih_context *c, CONST char *name, int type)
{
...
win32_appendmenu (struct uih_context *c, struct menurecord *menu)
{
    ...
    for (i = 0; (item = menu_item (menu->item->shortname, i)) != NULL; i++) {
        ...
        AppendMenu (menu->menu, flags, (submenu != NULL ? (UINT) submenu->menu : (UINT) (menu->id * IDMUL + i)), out);
                    ...
                    if (chgitem->flags & MENUFLAG_RADIO && checked) {
                        ...
                        for (y = 0; (item = menu_item (menu->item->shortname, y)) != NULL; y++)
                            if (item->flags & MENUFLAG_RADIO)
                                CheckMenuItem (menu->menu, menu->id * IDMUL + y, MF_BYCOMMAND | MF_UNCHECKED);
                    }
                    CheckMenuItem (menu->menu, menu->id * IDMUL + i, MF_BYCOMMAND | (checked ? MF_CHECKED : MF_UNCHECKED));
                }
            }
        menu = menu->nextrecord;
    }
