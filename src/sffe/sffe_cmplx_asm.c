...
sfarg *sflogCN(sfarg *const p)
{ /* logCN */
    sfvalue(p) =
        cdiv(sffecln(sfvalue(sfaram2(p))), sffecln(sfvalue(sfaram1(p))));
    return sfaram2(p);
};
...
{
    sfvalue(p) = sffecrtni(sfvalue(sfaram3(p)), (int)(sfvalue(sfaram2(p)).r),
