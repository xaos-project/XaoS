...
const sffunction sfcmplxfunc[sffnctscount] = 
	{ 
		...
		/*power functions*/
		{sfpow,2,"POW\0"},{sfpowi,2,"POWI\0"},{sfpowd,2,"POWD\0"},{sfpowdc,2,"POWDC\0"},
...
sfarg* sfrabs( sfarg * const p ) /* abs - real numbers*/
{
 sfvalue(p).r = sfvalue( sfaram1(p) ).r;
	if ( sfvalue(p).r<0 ) sfvalue(p).r = -sfvalue(p).r;
 sfvalue(p).i = 0;
 return sfaram1(p);
};
...
{
 sfvalue(p).r = sfvalue( sfaram1(p) ).r * (double)rand()/(double)RAND_MAX;
 sfvalue(p).i = 0;
 return sfaram1(p);
};

//const eval
void sfcPI( sfNumber *cnst ) { *cnst=cset(4*atan(1),0); };
void sfcPI2( sfNumber *cnst ) { *cnst=cset(2*atan(1),0); };
void sfc2PI( sfNumber *cnst ) { *cnst=cset(8*atan(1),0); };
void sfcE( sfNumber *cnst ) { *cnst=cset(exp(1),0); };
void sfcI( sfNumber *cnst ) { *cnst=cset(0,1); };
void sfcRND( sfNumber *cnst ) { *cnst=cset( (double)rand()/(double)RAND_MAX, 0 ); };
