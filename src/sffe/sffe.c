...
sffunction *
sffe_function (char *fn, size_t len)
{
    unsigned char idx;
...
void *
sffe_const (char *fn, size_t len, void *ptr)
{
    unsigned char idx = 3;
...
void *
sffe_regvar (sffe ** parser, sfNumber * vptrs, char vchars)
{
...
void *
sffe_regvars (sffe ** parser, unsigned int cN, sfNumber ** vptrs, char *vchars)
{
    unsigned int i = (*parser)->varCount;
    (*parser)->varCount += cN;
    (*parser)->varPtrs = (sfNumber **) realloc ((*parser)->varPtrs, (*parser)->varCount * sizeof (sfNumber *));
...
sfNumber *
sffe_varptr (sffe * const parser, char vchar)
{
    unsigned int i = 0;
...
sfNumber *
sffe_setvar (sffe ** parser, sfNumber * vptrs, char vchars)
{
    unsigned int i = 0;
    while (i < (*parser)->varCount) {
...
void *
sffe_regfunc (sffe ** parser, char *vname, unsigned int parcnt, sffptr funptr)
{
    sffunction *sff;
    unsigned short i;
...
void *
sffe_variable (sffe * const p, char *fname, size_t len)
{
    unsigned int idx = 0;
...
sffunction *
userfunction (const sffe * const p, char *fname, size_t len)
{
    unsigned char idx;
...
char
sffe_docmplx (char **str, sfarg ** arg)
...
int
sffe_parse (sffe ** parser, char *expression)
{
/**************var area */
    struct opstack__
    {
        ...
        if (p->errormsg)
            switch (err) {
                case MEMERROR:
                    ...
                    break;
                case UNBALANCEDBRACKES:
                    sprintf (p->errormsg, "Formula error ! UNBALANCED BRACKETS!! : %s", ch1);
                    break;
                case INVALIDFUNCTION:
                    sprintf (p->errormsg, "Formula error ! UNKNOWN FUNCTION!! : %s", ch1);
                    break;
                case INAVLIDNUMBER:
                ...
                case OPERATOR:
                    ...
                    break;
                case STACKERROR:
                    sprintf (p->errormsg, "Formula error ! INTERNAL STACK CORRUPTED!! : %s", ch1);
                    break;
                case PARCNTERROR:
                    sprintf (p->errormsg, "Formula error ! FUNCTION PARAMETERS ERROR!! : %s", ch1);
                    break;
                case NO_FUNCTIONS:
                    sprintf (p->errormsg, "Formula error ! ARE YOU KIDDING ME ?!? : %s", ch1);
                    break;
            };
        /* if error -> clean up */
        sffe_clear (&p);
