...
#ifndef USE_PNG
const char *
writepng (xio_constpath filename, const struct image *img)
{
...
writepng (xio_constpath filename, const struct image *image)
{
...
#ifdef _undefined_
    static char text[] = "XaoS" XaoS_VERSION " - a realtime interactive fractal zoomer";
    ...
    switch (image->palette->type) {
        case C256:
            {
                int i;
                png_set_IHDR (png_ptr, info_ptr, image->width, image->height, image->bytesperpixel * 8, PNG_COLOR_TYPE_PALETTE, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
        ...
        case LARGEITER:
        case GRAYSCALE:
            png_set_IHDR (png_ptr, info_ptr, image->width, image->height, image->bytesperpixel * 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
            break;
        case TRUECOLOR:
        case TRUECOLOR24:
        case TRUECOLOR16:
        ...
        case TRUECOLOR:
            {
                ...
                for (i = 0; i < image->height; i++) {
                    for (y = 0; y < image->width; y++)
                        r[y * 3 + 2] = (((pixel32_t **) (image->currlines))[i][y] & image->palette->info.truec.rmask) >> image->palette->info.truec.rshift, r[y * 3 + 1] = (((pixel32_t **) (image->currlines))[i][y] & image->palette->info.truec.gmask) >> image->palette->info.truec.gshift, r[y * 3] = (((pixel32_t **) (image->currlines))[i][y] & image->palette->info.truec.bmask) >> image->palette->info.truec.bshift;
        ...
        case TRUECOLOR16:
            {
                ...
                for (i = 0; i < image->height; i++) {
                    for (y = 0; y < image->width; y++)
                        r[y * 3 + 2] = (((pixel16_t **) (image->currlines))[i][y] & image->palette->info.truec.rmask) >> image->palette->info.truec.rshift, r[y * 3 + 1] = (((pixel16_t **) (image->currlines))[i][y] & image->palette->info.truec.gmask) >> image->palette->info.truec.gshift, r[y * 3] = (((pixel16_t **) (image->currlines))[i][y] & image->palette->info.truec.bmask) >> image->palette->info.truec.bshift;
                    png_write_rows (png_ptr, (png_bytepp) & r, 1);
                }
            }
            break;
