...
genertruecolorinfo (struct truec *t, unsigned int r, unsigned int g, unsigned int b)
{
    ...
    t->byteexact = 0;
    t->missingbyte = -1;
    if (!(t->rshift % 8) && !(t->gshift % 8) && !(t->bshift % 8) && !t->rprec && !t->gprec && !t->bprec) {
        t->byteexact = 1;
        {
            unsigned char ch[4];
...
struct palette *
createpalette (int start, int end, int type, int flags, int maxentries, int (*alloccolor) (struct palette * pal, int init, int r, int g, int b), void (*setcolor) (struct palette * pal, int start, int end, rgb_t * rgb), void (*allocfinished) (struct palette * pal), void (*cyclecolors) (struct palette * pal, int direction), union paletteinfo *info)
{
...
{
    struct palette *pal = createpalette (palette->start, palette->end, palette->type,
...
static INLINE void
hsv_to_rgb (int h, int s, int v, unsigned char *red, unsigned char *green, unsigned char *blue)
{
    int hue;
    int f, p, q, t;
    h += 256;
...
randomize_segments2 (int whitemode, int nsegments)
{
    ...
    for (i = 0; i < nsegments; i++) {
        if (i % 3 == 2)
            colors[i][0] = whitemode * 255, colors[i][1] = whitemode * 255, colors[i][2] = whitemode * 255;
        else if (i % 3 == 0)
            colors[i][0] = (!whitemode) * 255, colors[i][1] = (!whitemode) * 255, colors[i][2] = (!whitemode) * 255;
        else
            colors[i][0] = (int) XaoS_random () % 256, colors[i][1] = (int) XaoS_random () % 256, colors[i][2] = (int) XaoS_random () % 256;
    }
    colors[i - 1][0] = colors[0][0];
    colors[i - 1][1] = colors[0][1];
    colors[i - 1][2] = colors[0][2];
...
randomize_segments (int whitemode, int nsegments)
{
    ...
    if (whitemode) {
        ...
        for (i = 0; i < nsegments; i += 2) {
            ...
            if (i + 1 < nsegments)
                colors[i + 1][0] = (int) XaoS_random () % 35, colors[i + 1][1] = (int) XaoS_random () % 35, colors[i + 1][2] = (int) XaoS_random () % 35;
        }
    } else {
        for (i = 0; i < nsegments; i += 2) {
            colors[i][0] = (int) XaoS_random () % 35, colors[i][1] = (int) XaoS_random () % 35, colors[i][2] = (int) XaoS_random () % 35;
            if (i + 1 < nsegments)
                colors[i + 1][0] = (int) XaoS_random () % 256, colors[i + 1][1] = (int) XaoS_random () % 256, colors[i + 1][2] = (int) XaoS_random () % 256;
        }
    }
    colors[i - 1][0] = colors[0][0];
    colors[i - 1][1] = colors[0][1];
