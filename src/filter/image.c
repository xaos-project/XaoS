...
{
  unsigned char *data = (unsigned char *)calloc (((width + 3) & ~3) * height, bytesperpixel (palette->type));
  unsigned char *data1 = (unsigned char *)(nimages == 2 ? calloc (((width + 3) & ~3) * height, bytesperpixel (palette->type)) : NULL);
  struct image *img;
  if (data == NULL) {
#ifdef DEBUG
...
clear_image (struct image *img)
{
  ...
  if(img->palette->npreallocated) color=img->palette->index[0];
  if (!width) {
    width = (img->width + 7) / 8;
    if(color) color=255;
  }
