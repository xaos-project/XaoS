...
initialize (struct filter *f, struct initdata *i)
{
    ...
    size = sqrt (i->image->width * i->image->width * i->image->pixelwidth * i->image->pixelwidth + i->image->height * i->image->height * i->image->pixelheight * i->image->pixelheight);
...
doit (struct filter *f, int flags, int time)
{
    ...
    if ((val & CHANGED) || s->angle != angle) {
        ...
        drivercall (*f->image, xth_function (do_rotate8, f, f->image->height), xth_function (do_rotate16, f, f->image->height), xth_function (do_rotate24, f, f->image->height), xth_function (do_rotate32, f, f->image->height));
        xth_sync ();
        val |= CHANGED;
    }
    return val;
