...
doit (struct filter *f, int flags, int time)
{
    ...
    drivercall (*f->image, xth_function (do_starfield8, f, f->image->height), xth_function (do_starfield16, f, f->image->height), xth_function (do_starfield24, f, f->image->height), xth_function (do_starfield32, f, f->image->height));
    xth_sync ();
    return val | CHANGED;
}
