 ...
 *
 * Here are two different implementations of this algorithm - one is faster
 * and the second is threadable.
 *
 * The faster one is a quite usual implementation - get first uncalculated pixel,
 * trace algorithm using labyrinth "right hand rule" way (well, I currently
 ...
 * An threaded one follows description I sent to sci.fractals:
 ...
 > pushes new pixels on the processor to its left substack, and pops from
 ...
 calculate points just to distance 1. So I want to implement such algorithm,
 ...
 Problem is that this algorithm never ends, since boundaries will be rescanned
 ...
 Scan masks should be set in time when pixel is added to stack and pixel
 ...
 small and possible race conditions should not be drastically (except recalculating
 some point)
 ...
 in rest should be so small so this should not be drastically for less than 10
 processors)
 ...
 this cpu. This should avoid processor cache conflict and speed up process.
 ...
 queue should be more often situations when queue is empty since one processor
 is calculating points and other has wait, since it had tendency to trace
 just one boundary at the time. At the other hand, most of boundaries should
 cross borders, so they should be added to queue at the initialization, so
 this should be OK.

 I am beginner to threads, SMP etc. So I looking for ideas, and suggestions
 to improve this alg. (or design different one).
 Also someone with SMP box, who should test me code is welcomed.
 BTW what is the average/maximal number of CPU in today's SMP boxes?
 ...
 *    is quite full (there is more items than 10) processor just continues in
 ...
 *    are always correct, we should simply go trough each scanline and when
 *    pixel is uncalculated, copy value from its left neighbor
 *
...
#define MAXPAGES 200            /*Well limit is about 6MB of stack..Hope it will never overflow */
