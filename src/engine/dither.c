...
#define add1(i)                                                                \
    (next[(i)] == UNUSED ? (next[(i)] = LAST, next[last] = (i), (last = (i)))  \
                         : 0)

/* calculates distance between two colors */
#define dist(a1, a2, a3, b1, b2, b3)                                           \
    (col_diff[0][((a2) - (b2)) & 0x1FF] + col_diff[1][((a1) - (b1)) & 0x1FF] + \
...
#define better(r1, g1, b1, pal1)                                               \
    (((int)dist((r1), (g1), (b1), (int)(pal1)[0], (int)(pal1)[1],              \
                (int)(pal1)[2])) > (int)dist2)
...
#define dopos(rp, gp, bp, ts)                                                  \
    if ((rp > -1 || r > 0) && (rp < 1 || r < 248) && (gp > -1 || g > 0) &&     \
        (gp < 1 || g < 248) && (bp > -1 || b > 0) && (bp < 1 || b < 248)) {    \
        i = first + rp * 32 * 32 + gp * 32 + bp;                               \
        if (ts ? data[i] != val : !data[i]) {                                  \
            dist2 = (rp ? col_diff[1][(r + 8 * rp - (int)pal[val][0]) & 0x1FF] \
                        : r2) +                                                \
                    (gp ? col_diff[0][(g + 8 * gp - (int)pal[val][1]) & 0x1FF] \
                        : g2) +                                                \
                    (bp ? col_diff[2][(b + 8 * bp - (int)pal[val][2]) & 0x1FF] \
                        : b2);                                                 \
            if (better((r + 8 * rp), (g + 8 * gp), (b + 8 * bp),               \
                       pal[data[i]])) {                                        \
                data[i] = val;                                                 \
                add1(i);                                                       \
            }                                                                  \
...
static void create_rgb_table(unsigned char table[32][32][32],
                             struct palette *palette)
{
...
static void checksizes(unsigned char table[32][32][32], int *red, int *green,
                       int *blue)
{
    int r, g, b;
    int color;
...
static int initialize(struct filter *f, struct initdata *i)
{
    ...
    if (i->image->bytesperpixel <= 1) {
        int red, green, blue;
        if (!inherimage(f, i, TOUCHIMAGE /*| IMAGEDATA */, 0, 0, s->palette, 0,
                        0))
            return 0;
        if (!s->active) {
...
static struct filter *getinstance(const struct filteraction *a)
{
    ...
    i->palette = createpalette(0, 65536, TRUECOLOR, 0, 65536, NULL, NULL, NULL,
                               NULL, NULL);
    f->data = i;
...
static void convertgray(void *data, struct taskinfo *task, int r1, int r2)
{
    ...
    for (i = 0; i < 256; i++)
        table[i] = i * (img2->palette->end - img2->palette->start) / 256 +
                   img2->palette->start;
...
#define inten(x)                                                               \
    ((int)(((x)&255) * 76 + (((x) >> 8) & 255) * 151 +                         \
     ...
     8) -                                                                      \
        256
static void converttbitmap(void *data, struct taskinfo *task, int r1, int r2)
{
    struct filter *f = (struct filter *)data;
    struct image *img1 = f->childimage;
    struct image *img2 = f->image;
...
const struct filteraction truecolor_filter = {
    "Truecolor emulator", "truecolor",     0,
...
#ifdef SCONVERTORS
static int myfixedalloccolor(struct palette *p, int init, int r, int g, int b)
{
    ...
    if (i->image->palette->type == FIXEDCOLOR &&
        ...
        for (r = 0; r < MSIZE; r++)
            for (g = 0; g < MSIZE; g++) {
                s->rmat[r][g] = ((int)matrix[r][g] - 128) * red / 256;
                s->gmat[r][g] =
                    ((int)matrix[(r + 3) % MSIZE][(g + 6) % MSIZE] - 128) *
                    green / 256;
                s->bmat[r][g] =
                    ((int)matrix[(r + 6) % MSIZE][(g + 3) % MSIZE] - 128) *
                    blue / 256;
            }
...
const struct filteraction fixedcolor_filter = {
    "Palette emulator", "fixedcolor",         0,
    getinstancefixed,   destroyinstancefixed, doitfixed,
    requirement,        initializefixed,      convertupgeneric,
    convertdowngeneric, myremovefilterfixed,
};
#endif
...
static void convertbitmap(void *data, struct taskinfo *task, int r1, int r2)
{
    struct filter *f = (struct filter *)data;
    struct image *img1 = f->childimage;
    struct image *img2 = f->image;
