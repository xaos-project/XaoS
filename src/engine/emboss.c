    ...
    if (datalost (f, i) || i->image->version != f->imageversion) {
        ...
        if (i->image->palette->type & (C256 | GRAYSCALE)) {
            ...
            for (x = 0; x < 256; x++) {
                ...
                if (dist > 255)
                    dist = 255;
                s->table[x] = ((dist >> i->image->palette->info.truec.rprec) << i->image->palette->info.truec.rshift) | ((dist >> i->image->palette->info.truec.gprec) << i->image->palette->info.truec.gshift) | ((dist >> i->image->palette->info.truec.bprec) << i->image->palette->info.truec.bshift);
...
getinstance (const struct filteraction *a)
{
    ...
    i->savedpalette = NULL;
    i->palette = createpalette (0, 256, GRAYSCALE, 0, 256, NULL, NULL, NULL, NULL, NULL);
    f->childimage = NULL;
    f->data = i;
    f->name = "Emboss";
    return (f);
...
doit (struct filter *f, int flags, int time1)
{
    ...
    drivercall (*f->image, xth_function (emboss8, f, f->image->height), xth_function (emboss16, f, f->image->height), xth_function (emboss24, f, f->image->height), xth_function (emboss32, f, f->image->height));
    xth_sync ();
    return val;
}
