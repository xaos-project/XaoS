...
uih_line (uih_context * c, dialogparam * d)
{
  ...
  if (c->playstring != NULL)
    {
      seterr ("line available only in animation replay");
      return;
    }
...
uih_morphline (uih_context * c, dialogparam * d)
{
  ...
  if (l == NULL)
    {
      seterr ("Morphing non existing line!");
      return;
    }
...
uih_setkey (uih_context * c, int line)
{
  if (!c->play)
    {
      seterr ("linekey not available in this context!");
      return;
    }
...
uih_clear_line (uih_context * c)
{
  if (c->playstring != NULL)
    {
      seterr ("clear_line available only in animation replay");
      return;
    }
  uih_removeline (c, uih_findkey (c, c->playc->lines.currkey++));
}
...
uih_clear_lines (uih_context * c)
{
  if (c->playstring != NULL)
    {
      seterr ("clear_lines available only in animation replay");
      return;
    }
...
uih_loadcatalog (uih_context * c, CONST char *name)
{
  ...
  if (f == XIO_FAILED)
      ...
      if (firsttime)
	{
      ...
      uih_error (c, "Catalog file not found");
      return 0;
    }
  firsttime = 0;
  if (catalog != NULL)
		  ...
		  int animr)
{
  ...
  if (uih->play)
    {
      uih_error (uih, "Replay is arelady active");
      return 0;
    }
  if (f == XIO_FAILED)
    {
...
uih_playmorph (struct uih_context *uih, dialogparam * d)
{
  if (uih->playstring != NULL)
    {
      seterr ("morph available only in animation replay");
      return;
    }
...
uih_playmove (struct uih_context *uih, number_t x, number_t y)
{
  if (uih->playstring != NULL)
    {
      seterr ("move available only in animation replay");
      return;
    }
  uih->playc->source = uih->fcontext->s;
...
uih_playmorphjulia (struct uih_context *uih, number_t x, number_t y)
{
  if (uih->playstring != NULL)
    {
      seterr ("morphjulia available only in animation replay");
      return;
    }
...
uih_playmorphangle (struct uih_context *uih, number_t angle)
{
  if (uih->playstring != NULL)
    {
      seterr ("morphangle available only in animation replay");
      return;
    }
...
uih_playusleep (struct uih_context *uih, int time)
{
  ...
  if (uih->playstring != NULL)
    {
      seterr ("sleep available only in animation replay");
      return;
    }
...
uih_playload (struct uih_context *uih, xio_path file)
{
  ...
  if (uih->playstring != NULL)
    {
      seterr ("load available only in animation replay");
      return;
    }
