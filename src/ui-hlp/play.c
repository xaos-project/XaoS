...
skipblank (struct uih_context *uih)
{
    ...
    if (uih->playstring != NULL) {
        while ((uih->playstring[uih->playpos] == ' ' || uih->playstring[uih->playpos] == '\t' || uih->playstring[uih->playpos] == '\r' || uih->playstring[uih->playpos] == '\n'))
            uih->playpos++;
        return;
    }
    do {
...
uih_playupdate (struct uih_context *uih)
{
    ...
    while (uih->play && uih->playc->playframe && errstring == NULL) {
        ...
        if (errstring != NULL) {
            ...
            if (uih->play) {
                sprintf (errtext, gettext ("Replay disabled at line %i"), uih->playc->line);
