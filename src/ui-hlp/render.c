		 ...
		 struct frame_info *f2)
{
  ...
  if (f == NULL)
    {
      x_error ("Can not open motion vector file!");
      return;
    }
		     ...
		     CONST char *catalog, int motionvectors, int iframedist2)
{
  ...
  printmsg ("Vectors:%i", motionvectors);
  if (iframedist2)
  ...
  printmsg ("Enabling animation replay\n");
  ...
  if (!gc)
    x_message ("Entering calculation loop!");
  else
    printmsg ("Entering calculation loop!");
  ...
  if (interrupt)
    ...
    {
      if (!gc)
	x_message ("Caluclation finished");
      else
