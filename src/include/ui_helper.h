...
#define UNDOLEVEL 256
    ...
    typedef void (*uih_getposfunc) (struct uih_context * c, int *x, int *y, int *width, int *height, void *data);
...
#define uih_updatemenus(uih,c) if(uih->updatemenus!=NULL) uih->updatemenus(uih,c);
    ...
    struct uih_context *uih_mkcontext (int flags, struct image *image, int (*passfunc) (struct uih_context *, int, const char *, float), void (*longwait) (struct uih_context *), void (*updatemenus) (struct uih_context * c, const char *));
...
/*misc functions */
    ...
    void uih_screentofractalcoord (uih_context * c, int mousex, int mousey, number_t * re, number_t * im);
...
/*Animation save routines */
    ...
    int uih_replayenable (struct uih_context *uih, xio_file f, xio_constpath filename, int animroot);
...
/*Windows :)*/
    struct uih_window *uih_registerw (struct uih_context *uih, uih_getposfunc getpos, uih_drawfunc draw, void *data, int flags);
    ...
    void uih_drawborder (struct uih_context *uih, int x, int y, int width, int height, int flags);
    void uih_setline (struct uih_context *uih, struct uih_window *w, int color, int x1, int y1, int x2, int y2);
    struct uih_window *uih_registerline (struct uih_context *uih, int color, int x1, int y1, int x2, int y2);
