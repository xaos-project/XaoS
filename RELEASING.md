...
* Change version number in doc/XaoS.lsm (also the date)
* If the Windows version is built manually (otherwise optional):
  - Edit installer/config/config.xml and installer/xaos.iss (if the latter is changed, also change Dockerfile.win)
